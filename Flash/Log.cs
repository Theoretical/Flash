...
namespace Flash
{
    public class Log
    {
        ...
        private static volatile object _invokeLock;

        public static void Initialize()
        {
            _textWriter = Console.Out;
            _streamWriter = new StreamWriter("Summoning.txt");
            _invokeWriter = new StreamWriter("Invoke Log.txt");
            _oLock = new object();
            _invokeLock = new object();
        }
        ...
        public static void Invoke(string service, string method)
        {
            lock (_invokeLock)
            {
                _invokeWriter.WriteLine("[{0}] {1}::{2}", DateTime.Now, service, method);
                _invokeWriter.Flush();
            }
        }
